...
**테스트 작성.** 항목에 테스트가 필요할 때 `loop-run`은 절대 maker가 자신의 코드를 위한 테스트를 직접 작성하게 두지 않습니다 — 작성자는 `config.tests.author`로 선택합니다: **codex-cli** ([OpenAI Codex CLI](https://github.com/openai/codex), `codex exec`), **codex-plugin** (마켓플레이스 플러그인 `codex @ openai-codex`, Claude Code 세션 필요), 또는 **subagent** (컨텍스트가 격리된 Claude 에이전트 — 기본값이며, 선택된 작성자를 사용할 수 없거나 실패할 때의 대체 수단). 어느 쪽이 작성하든 동작 명세만 전달받고 maker의 추론은 전혀 보지 않으므로, 테스트는 구현이 아니라 동작을 검증합니다.
...
- **Phase 4 — 확장성** ✅: 플러그블 테스트 작성자(Codex CLI / 마켓플레이스 플러그인 / subagent), 타겟팅된 `/loop-run #123`, prompt 기반 `loop-task` write-back.
