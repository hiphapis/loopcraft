...
**테스트 작성.** 항목에 테스트가 필요할 때 `loop-run`은 절대 maker가 자신의 코드를 위한 테스트를 직접 작성하게 두지 않습니다. 테스트는 기본적으로 설치되어 있고 설정된 **[Codex](https://github.com/openai/codex)**(OpenAI의 Codex CLI)가 작성합니다 — Codex를 사용할 수 없는 경우에는 loopcraft가 **컨텍스트가 격리된 서브에이전트**(동작 명세만 전달받고 maker의 추론은 전혀 보지 않는 새 Claude 에이전트)로 대체합니다. 어느 경우든 테스트는 독립적으로 작성되어, 구현이 아니라 동작을 검증합니다. Codex를 사용하려면 [Codex CLI](https://github.com/openai/codex)를 설치하고 로그인하세요.
