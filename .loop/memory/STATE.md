...
## Next steps
- #5 Jira adapter (`adapters/jira.sh`): the last Phase 3 piece. Now unblocked by #4 (write-back seam done — PR creation is a separate `config.pr`, so a Jira `report` composes with a GitHub `pr`). Design-first: decide tooling/auth (jira-cli / acli / curl+REST); write-back = comment + status transition (no PRs in Jira).
