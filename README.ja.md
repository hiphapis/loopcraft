...
**単一アイテムの指定。** 裸の数値（`/loop-run 3`）はキュー実行のアイテム数上限であり、セレクター（`/loop-run #123`、または Jira キー）はそのアイテム 1 つだけをフルサイクルで実行します。`config.backlog.get` を介してアイテムを取得し — アイテムが `loop:ready` でない場合は、進める前に確認を求めます（ターゲットモードは対話的です）。
